...
function setupToDHTMLandData(){
	for (var c = 0; c < 3; c++) {
		...
		var branchUpgrades = ["獲得 <span id='" + color + "UpgPow1'></span>x " + color + " 夸克自旋，但是 " + color + " 夸克的衰變速度快 <span id='" + color + "UpgSpeed1'></span>x。",
				      ...
				      (["Red", "Green", "Blue"])[c]+" <span id='" + color + "UpgName3'></span> 夸克的衰變速度慢<span id='" + color + "UpgEffDesc'> 4x</span>。"] //might need to change this to just "slower" once we have 1000+ upgrade 3's
		...
		for (var u = 1; u <= 3; u++) {
			...
			html += (u == 2 ? "<br><button class='storebtn' style='width: 190px' onclick='maxBranchUpg(\"" + shorthand + "\")'>最大購買所有升級</button>" + "<br><button class='storebtn' style='width: 190px; font-size:10px' onclick='maxBranchUpg(\"" + shorthand + "\", true)'>最大購買第二和第三升級</button>":"")+"</td>"
		}
		html += "</tr></tr><td></td><td><button class='gluonupgrade unavailablebtn' id='" + shorthand + "RadioactiveDecay' style='font-size:9px' onclick='radioactiveDecay(\"" + shorthand + "\")'>重置並加強第一個升級，但削弱這個分支。<br><span id='" + shorthand + "RDReq'></span><br>輻射衰變：<span id='" + shorthand + "RDLvl'></span></button></td><td></td>"
		html += "</tr></table>"
...
function setupBosonicExtraction(){
	...
	for (var g2 = 2; g2 <= br.maxLimit; g2++) {
		...
		for (var g1 = 1; g1 < g2; g1++) {
			...
			"價格：<span id='bEnG1Cost" + id + "'></span> <div class='bRune' type='" + g1 + "'></div> & <span id='bEnG2Cost" + id + "'></span> <div class='bRune' type='" + g2 + "'></div></button><br>"
		}
	}
	var toeDiv = ""
...
function setupBosonicUpgrades(){
	...
	for (r = 1; r <= bu.maxRows; r++) {
		...
		for (c = 1; c < 6; c++) {
			...
			"價格：<span id='bUpgCost" + id + "'></span> 玻色子反物質<br>" +
			"需要：<span id='bUpgG1Req" + id + "'></span> <div class='bRune' type='" + bu.reqData[id][2] + "'></div> & <span id='bUpgG2Req" + id + "'></span> <div class='bRune' type='" + bu.reqData[id][4] + "'></div></button>"
		}
	}
}
...
function setupBosonicRunes(){
	...
	for (var g = 0 ; g < glyphs.length; g++) {
		...
		if (type > 0 && type <= br.maxLimit) {
			...
			glyph.setAttribute("ach-tooltip", br.names[type] + "玻色子符文")
		}
	}
}
